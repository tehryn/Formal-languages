	...
	while (L->Active!=NULL)
    {
		...
		switch (L->Active->type_instr)
		{
			case I_ASSIGMENT:
				...
				new=do_expression(postfix_array,I_Htable,S,L,&fce_stack);
			...
			case I_FCE:
				...
				if (strcmp(return_hitem->key,"ifj16.print")==0)
				{
					//printf("ASDF\n");
					char *help_tmp;
					token *str_token=do_expression(postfix_array,I_Htable,S,L,&fce_stack);
...
//					print("\n");
			...
			case I_RETURN:
				...
				L->Active=stack_instr_pop(&fce_stack);
				
				//I_Instr *asdff1=stack_instr_pop(&fce_stack);
					//printf("POP1 L->Active->next: %d\n",asdff1->type_instr);
...
token *do_expression(token *postfix_array, stack_htab *I_Htable,struct stack_expresion *S,Instr_List *L,stack_instr *fce_stack)
{
